...
**Image references:**
- Image fill nodes → use the exact PNG path or basename returned by `export_nodes`
- SVG decorative nodes → use the exact SVG path or basename returned by `export_nodes`
- Never derive an exported filename from the node ID; preserve the runtime-generated timestamp
