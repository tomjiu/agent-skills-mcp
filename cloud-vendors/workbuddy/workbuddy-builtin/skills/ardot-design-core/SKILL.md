...
| `{SKILL_ROOT}/rules/design-rules.md` | **Single source of truth** — editing principles, coordinates, flexbox, text, colors, tables, images, effects, SVG, property schema, troubleshooting, post-generation validation |
...
| `{SKILL_ROOT}/func-rule/component-instance/SKILL.md` | **Components, instances, variants** — insert/update refs, `componentProperties`, VARIANT, nested instance overrides, `mainComponent` swap. **MUST load** before editing any `INSTANCE` / `COMPONENT` / `COMPONENT_SET`, or when the user asks to switch state / variant |
| `{SKILL_ROOT}/func-rule/design-variables/SKILL.md` | Bind/unbind variables (`$:Set:Var` / `$<id>`), `apply_variables`, and node `variableModes` (theme / density). Load when binding tokens or switching modes. |
| `{SKILL_ROOT}/func-rule/shared-styles/SKILL.md` | Bind/unbind shared styles (`textStyleId` / `fillStyleId` / …). Load when applying style ids. |
...
> ⛔ **HARD RULE — User guidance wins on conflict.** When user-provided style guidance (e.g. free-text style/visual constraints in the user message, `DESIGN.md`, design tokens) conflicts with a loaded guideline, product-type prior, **or a selected `<ardot_design_style>` / style-template**, **always follow the user**. Templates and built-in guidelines only fill unspecified gaps — never override the user's visual system.

**Style-template shortcut — if `<ardot_design_style>` block is present:** the user picked a style template through the chip. Follow the injected `[style-template]` instruction: `curl` the referenced md URL, use that template as the **base** visual system, and **SKIP Step 4 and Step 5** (`search_style_guide` / `build_style_guide`) — no separate discovery flow is needed. If the user message also states explicit style/visual constraints, **override conflicting template dimensions with the user's guidance**; keep template values only where the user did not specify.
...
Use `capture_screenshot` and download the return image. These are internal verification artifacts — never surface their paths to the user, never write them elsewhere (`/tmp`, home dir, project root).
