...
_PATH_BATCH_ADD_RECORDS = "/space/api/agent/v1/batch-add-records"
...
def main() -> None:
    ...
    for index, record in enumerate(records):
        ...
        validated = validate_properties(record)
        if not validated:
            error_exit(f"records[{index}] 校验后为空，无有效字段")
        validated_records.append(validated)

    body = {"databaseId": database_id.strip(), "records": validated_records}
    try:
        response = db_post(
            _common.build_url(_PATH_BATCH_ADD_RECORDS), token, body, timeout=15.0
        )
    except HttpError as exc:
        error_exit(f"批量添加记录请求失败: {exc}", traceid=exc.traceid)
