...
> - `cwd` 需要设为 `<plugin_root>/skills/html-to-docx/scripts`（`html_to_docx` 包在此），或用 `PYTHONPATH=<plugin_root>/skills/html-to-docx/scripts`。
> - 需要把 venv 落到别处时，调用前 `export HTML_TO_DOCX_VENV=/自定义/路径` 即可。
> - **禁止**在 `<plugin_root>/skills/html-to-docx/scripts/` 下自建 `.venv/`——那是过去踩过的坑（跨机器 shebang 失效、rm 被 safe-delete 拦）。
> - Plugin 部署态下，SessionStart hook 已经自动跑过一次安装脚本，agent 调用时通常无需再手动 bash。

**依赖清单**：以 `<plugin_root>/skills/html-to-docx/scripts/requirements.txt` 为准，`setup-html-to-docx.sh` 用 `--only-binary=:all:` 强制装 wheel，规避 lxml 在无 libxml2/libxslt 环境下源码构建失败。
...
# 方式 2：用 PYTHONPATH
PYTHONPATH=<plugin_root>/skills/html-to-docx/scripts "$HTML_TO_DOCX_PY" -m html_to_docx convert input.html -o output.docx [OPTIONS]
```

**路径变量说明**：
- `<plugin_root>` = plugin 根目录的绝对路径（即产物里的 `tencent-docx/` 所在位置）
- `<workspace>` = 用户工作区目录（仅用于落盘产物路径，如 `<workspace>/output/<request_id>/output.docx`）

> **Agent 集成提示**：在 subprocess 调用时，推荐使用 `cwd = path.join(pluginRoot, "skills/html-to-docx/scripts")` + `executable = $HTML_TO_DOCX_PY`，无需设置 PYTHONPATH。**不要**使用系统 `python` / `python3`——依赖装在托管 venv 里，系统解释器会 `ModuleNotFoundError`。
