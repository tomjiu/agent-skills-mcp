...
```jsx
<Image src="assets/hero.png" style={{ width: 400, borderRadius: 12 }} />
```

```jsx
<Image
    src="assets/ai_concept.png"
...
| 属性 | 说明 |
| :--- | :--- |
| `src` | **必填**。本地相对路径（相对项目根），必须先保存到 `assets/`。**严禁** `http://` / `https://` / `image://` 占位符 |
...
用户上传材料、`scripts/extract.py` 抽取图、用户明确提供的品牌 / 产品 / 人物 / 场景图片可作为配图来源。使用前必须复制到 `assets/`，重命名为英文 / 数字 / 下划线文件名，并逐张 Read 核对内容、比例、清晰度。
...
```jsx
<Image src="assets/team_collab.png" style={{ width: 400, height: 225 }} />
// 宽高比按实际图片比例（1920/1080 ≈ 16:9 → 400×225）
```

## 严禁事项

- **不得用通配符删除图片资源**（如 `rm -rf assets/*.png`）。替换图片时优先覆盖明确文件；确需删除时逐个指定具体路径。
