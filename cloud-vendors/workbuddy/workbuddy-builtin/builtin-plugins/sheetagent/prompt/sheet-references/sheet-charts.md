...
##### 4.3.1.0b-1 Automatic collision avoidance when creating multiple charts

> **`newChart()` automatically avoids overlapping existing charts and data:**
> it probes `sheet.getCharts()` before writing and, if the requested anchor collides with an
> existing chart or the sheet's used range, **auto-adjusts the anchor** (prefers the first free column
> right of the data, then below the data, then chart-only avoidance). The script log will report the
> adjustment (e.g. `[newChart] 图表位置调整: {"autoAdjusted":true,...}`) so you can rely on the final
> position without recomputing it yourself.
>
> **Default behavior (`explicitLayout` omitted / `false`)**: position is treated as a *hint* — collisions
> are auto-resolved. Use this for "generate line, pie, and bar charts for the data" (multiple charts,
> any arrangement is fine).
>
> **Explicit placement (`explicitLayout: true` in the options object)**: the anchor is treated as a
> user-specified final position — the engine **will NOT move it**, even if it overlaps a chart or data;
> a warning + suggested location is returned instead. Only pass `explicitLayout: true` when the user
> literally named a location in natural language (e.g. "put the pie chart at H2"). For everything else
> leave it as `auto` (the default).
>
> ```javascript
> // auto (default): collisions are auto-resolved — safe for "generate 3 charts" prompts.
> const sheet = SpreadsheetApp.getActiveSheet();
> const dataRange = sheet.getRange(1, 1, 9, 5);
> const line = sheet.newChart('chart_line', 'LINE', dataRange, 0, 5, 0, 0, 480, 300);
>
> // explicit: user named a specific spot — engine respects it, never auto-moves.
> const pie = sheet.newChart('chart_pie', 'PIE', dataRange, 0, 7, 0, 0, 480, 300,
>   { explicitLayout: true, title: 'Pie' });
> ```
>
> **Auto-extend when the target is beyond the current sheet size**: if the (requested or avoided)
> anchor would place the chart beyond the sheet's existing rows/columns, the shim **automatically
> inserts the needed rows/columns** (`insertRows` / `insertColumns`) before placing the chart. You do
> **NOT** need to extend the sheet yourself — just pass the anchor you want; the engine grows the sheet
> as needed (bounded to avoid pathological growth).
