...
# ── 身份契约（issue #76568）─────────────────────────────────────────────────
# C 端只认 OAuth 票据（Authorization: Bearer）。SaaS OneID 票据仅在「双票」场景
# 作附带项；只有 OneID 票时打 C 端端点必然 401 invalid_token。
REQUIRED_TICKET = "oauth"
IDENTITY_LABEL = "个人版（QQ / 微信，C 端 OAuth 票据）"
PEER_SKILL = "tencent-saas-docs"
PEER_IDENTITY_LABEL = "企业版（SaaS，OneID 票据）"
MCP_URLS = {
