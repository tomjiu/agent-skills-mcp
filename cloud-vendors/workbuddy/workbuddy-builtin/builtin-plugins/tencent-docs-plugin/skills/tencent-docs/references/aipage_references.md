...
```bash
python3 <skill_dir>/tencentdocs.py tdoc_call tencent-docs manage.pre_import \
  '{"file_name": "<basename(AIPAGE_PATH)>", "file_size": <AIPAGE_SIZE>, "file_md5": "<AIPAGE_MD5>"}'
```

返回字段中需要：`upload_url`、`file_key`、`task_id`。

> 也可以直接复用 `import_file.py`（位于本 skill 同目录，纯 Python / 跨平台），它已封装 Step 2 之后的
> 「pre_import + PUT 上传 COS」两步，输出 `IMPORT_READY` + 关键字段。
> 推荐写法：先用 `node aipage_pack.js` 打出 `.aipage`，再 `python3 import_file.py <AIPAGE_PATH>`。

### Step 3：PUT 上传到 COS

跨平台做法是直接用 `import_file.py`（已封装 pre_import + PUT，见下方模板）。
若需手动 PUT，Python 一行即可（Windows 无需 curl）：

```bash
python3 <skill_dir>/import_file.py "<AIPAGE_PATH>"   # 推荐：自动完成 pre_import + PUT
```
...
```bash
# 触发
python3 <skill_dir>/tencentdocs.py tdoc_call tencent-docs manage.async_import \
  '{"task_id":"<task_id>","file_key":"<file_key>","file_name":"<file_name>","file_md5":"<AIPAGE_MD5>","file_size":<AIPAGE_SIZE>}'

# 轮询（建议每 3s 一次，最多 60s）
python3 <skill_dir>/tencentdocs.py tdoc_call tencent-docs manage.import_progress '{"task_id":"<task_id>"}'
```
...
# ② + ③ pre_import + PUT（直接复用 import_file.py，纯 Python / 跨平台）
IMPORT_OUT=$(python3 <skill_dir>/import_file.py "$AIPAGE_PATH")
