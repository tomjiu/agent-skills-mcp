 ...
 *
 * 依赖：
 *   - Node.js (>= 14) 与 Python 3
 *   - 同目录下的 tencentdocs.py（纯 Python 调 MCP HTTP/JSON-RPC，从环境变量读 token）
 */
...
const TDOC_PY = path.resolve(__dirname, "tencentdocs.py");
// Windows 上 Python 解释器通常叫 python，类 Unix 上叫 python3；优先 python3。
...
// ── MCP 调用封装：经由 tencentdocs.py tdoc_call（纯 Python，跨平台）────────────
...
function mcpCall(tool, argsObj) {
  ...
  try {
    stdout = execFileSync(
      PYTHON,
      [TDOC_PY, "tdoc_call", MCP_SERVICE, tool, argsJson],
      { encoding: "utf-8", timeout: 120000 }
    );
  } catch (err) {
    var msg = err.stderr || err.stdout || err.message || "unknown error";
    throw new Error("tdoc_call " + tool + " failed: " + msg);
  }

  var rsp;
  try {
    rsp = JSON.parse(stdout.trim());
  } catch (e) {
    // 服务端不是 JSON（这不应该发生，MCP 协议始终返 JSON-RPC）原样返回文本
    return stdout.trim();
  }

  if (rsp && rsp.error) {
    var e = rsp.error;
    throw new Error("tool " + tool + " returned error: code=" + e.code + ", msg=" + e.message);
  }
  var result = rsp && rsp.result;
  if (!result) {
    return stdout.trim();
  }
  // 优先 structuredContent；后退到 content[0].text
