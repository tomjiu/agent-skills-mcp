...
The 5-key theme system (`primary`, `secondary`, `accent`, `light`, `bg`) does NOT assume light backgrounds. When the template or chosen palette is dark-mode:

- `bg` = dark background color (e.g., `1A1A1A`, `000814`)
- `primary` = light text color (e.g., `FFFFFF`, `E6E6E6`)
- `light` = slightly lighter dark color for card/panel backgrounds (e.g., `2E2E2E`, `001D3D`)
- `secondary` = muted light color for secondary text (e.g., `A0A0A0`, `737373`)
- `accent` = accent color (same role as light-mode)

**How to detect dark-mode**: If the slide background uses a color with luminance < 30% (hex value components mostly below `4D`), treat as dark-mode. When imitating a template, check the actual slide screenshots — dark backgrounds with light text = dark-mode.

```javascript
// Dark-mode theme example (VS2019-style)
const theme = {
  primary:   'FFFFFF',   // Light text on dark background
  secondary: '737373',   // Muted text
  accent:    '5C2D91',   // Purple accent
  light:     '2E2E2E',   // Card/panel background (slightly lighter than bg)
  bg:        '1A1A1A'    // Dark slide background
};

// Light-mode theme example (default)
const theme = {
  primary:   '22223b',   // Dark text on light background
  secondary: '4a4e69',   // Muted text
  accent:    '9a8c98',   // Accent
  light:     'c9ada7',   // Card/panel background
  bg:        'f2e9e4'    // Light slide background
};
```
