...
5. **Edit content**: Update text in each `slide{N}.xml`.
   **Use separate Mavis sessions here if available** — slides are separate XML files, so worker sessions can edit in parallel via `mavis session new`.
...
**Worker sessions:** If available, use separate Mavis sessions here (after completing step 4). Each slide is a separate XML file, so worker sessions can edit in parallel via `mavis session new`. In your prompt to each worker session, include:
